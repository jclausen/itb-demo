...
<cffunction name="HeaderRowDelimitedToQuery">
		...
		<cfloop list="#data#" index="csv.line" delimiters="#csv.newLine#">
			...
			<cfscript>
				...
				*/
				...
				{
					csv.header = this.csvLineToArray( trim(csv.line), delimiter );
					
					for ( loc.i = 1; loc.i LTE arrayLen(csv.header); loc.i = loc.i + 1 )
						//csv.header[loc.i] = rereplacenocase( csv.header[loc.i], "[^a-z0-9]", "", "ALL" );
						csv.header[loc.i] = ucase(rereplacenocase(trim(csv.header[loc.i]),'[^a-zA-Z0-9]','_','ALL'));
				...
				{	
					csv.lineArr = this.csvLineToArray( trim(csv.line), delimiter );
					...
					if ( arraylen(csv.lineArr) EQ arrayLen(csv.header) )
					{
						...
						for ( loc.i = 1; loc.i LTE arrayLen(csv.header); loc.i = loc.i + 1 )
						{
							querySetCell( csv.query, ucase(rereplacenocase(trim(csv.header[loc.i]),'[^a-zA-Z0-9]','_','ALL')), csv.lineArr[loc.i] );
						}
					}
					else
					{
