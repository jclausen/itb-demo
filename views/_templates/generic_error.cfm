...
<cfscript>
var Mailer = application.wirebox.getInstance("MailService@cbmailservices");
var AppSettings = application.wirebox.getBinder().getProperties();
var message = Mailer.newMail(
	to=AppSettings.error_to,
	from=AppSettings.error_from,
	subject="[#AppSettings.site_title#] Error on Page: #event.getCurrentRoutedURL()#"
	);
message.setHTML(bugReport);
Mailer.send(message);
</cfscript>
<!--- <cfif !event.isAjax()> --->
<cfoutput>
	<cfinclude template="../../layouts/Error.cfm">
</cfoutput>	
<!--- </cfif> --->
